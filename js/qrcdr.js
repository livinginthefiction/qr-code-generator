    ...
    $.fn.extend({
        ...
        qrcdr: function(options) {
            ...
            return this.each(function() {
                ...
                plugin.updateCode = function(saveQR) {
                    return updateCode(saveQR);
                };
                 ...
                 */
                function updateCode(saveQR= false) {
                    $('.toast').toast('hide');
                    clearTimeout(timer);
                    ...
                    timer = setTimeout(function(){
                        ...
                        preloader.fadeIn(100, function(){
                            ...
                            if (saveQR) {sendata = sendata + '&save=TRUE&token='+sessionStorage.getItem('token');} 
                            else {sendata = sendata + '&save=FALSE&token='+sessionStorage.getItem('token');}
                            console.log(sendata);
                            ...
                            .done(function(msg) {
                                // console.log('shubham',JSON.parse(msg).token);
                                if (JSON.parse(msg).token) {sessionStorage.setItem('token', JSON.parse(msg).token)}
                                if (transparent_bg.prop('checked')) {
                                    colorpickerback.spectrum('disable');
                                }
